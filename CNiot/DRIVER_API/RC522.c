...
void IC_CMT(uchar *UID,uchar *KEY,uchar RW,char *Dat)
{
  ...
  if(RW)//读写选择，1是读，0是写
  {
    PcdRead(0x10,(unsigned char*)Dat);
     //UartSend (PcdRead(0x10,Dat));
  }
  else 
  {
    PcdWrite(0x10,(unsigned char*)Dat);
...
char* IC_ReadUID(void)
{
  static uchar ucTagType[4];
  static uchar find=0xaa;
  static uchar ret,i;
  static char Card_Id[8]; //存放32位卡号
